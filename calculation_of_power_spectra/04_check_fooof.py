...
coord_dir = '.../Parcellation/Glasser52/Parcellation_Info/';
...
#%% Parcels x Participants

metrics = [err, rsq, offsets, aperiodic_exponent]
out_metrics = [out_err, out_rsq, out_offsets, out_aperiodic_exponent]
...
for metric, out_metric, metric_label in zip(metrics, out_metrics, metric_labels):
    ...
    group_long = np.tile(group_list,52)
    metric_long = metric.flatten('F')
    state_long = np.repeat(ROIy_rank,len(metric))
    
    df = pd.DataFrame(np.vstack([group_long,metric_long,state_long]).T,columns=['group','metric','parcel'])
    
    # Bring Outlier Data into long format
    outlier_long = np.tile(outlier_list,52)
    out_metric_long = out_metric.flatten('F')
    out_state_long = np.repeat(ROIy_rank,len(out_metric))
    
    df_out = pd.DataFrame(np.vstack([outlier_long,out_metric_long, out_state_long]).T,columns=['group','metric','parcel'])
    
    # --- Make Split Bar Plot for all States - w Stripe Plots ---
    
    # Start Plotting
    fig = plt.figure(dpi=300, figsize=(7.4,4.8))
    ax = fig.add_subplot()
                   
    # make plots
    sns.boxplot(x="group", y='metric', data=df, palette=group_colors, width=.5, ax = ax, showfliers=False)
    for i in range(4):
        sns.stripplot(x="group", y='metric', hue='parcel',data=df[df['group']==i+1], 
                      palette=cmaps[i], size=2, dodge=False, 
                      legend=False,ax = ax)
        
        # if i == 0:
        #     sns.stripplot(x="group", y='metric', hue='parcel',data=df_out[df_out['group']==i+1], 
        #                   color="red", size=4, dodge=False, 
        #                   legend=False,ax = ax)
    ...
    entry_inds = np.argsort(ROIy_rank)[np.arange(0,len(ROIy_rank),15)]
    legend_values = z_yrank[entry_inds]
    legend_labels = np.round(ROIy[entry_inds]).astype(int).astype(str)
    ...
    # # Save Plot    
    plt.savefig(f'{plot_dir}/{metric_label}_overview.png',
                bbox_inches = 'tight')
...
for metric, out_metric, metric_label in zip(metrics, out_metrics, metric_labels):
    ...
    # # Save Plot    
    plt.savefig(f'{plot_dir}/{metric_label}_parcel_overview.png',
                bbox_inches = 'tight')
...
metrics = [err.mean(axis=0), rsq.mean(axis=0), offsets.mean(axis=0), aperiodic_exponent.mean(axis=0)]
metric_labels = ['Error','R-Squared','Offset','Aperiodic Exponent']
...
for metric, metric_label in zip(metrics, metric_labels):
    ...
    # Bring Data into long format
    metric_long = metric.flatten('F')
    state_long = ROIy_rank
    
    df = pd.DataFrame(np.vstack([metric_long,state_long]).T,columns=['metric','parcel'])
    ...
    # make plots
    sns.boxplot(y='metric', data=df, palette=group_colors, width=.5, ax = ax, showfliers=False)
    sns.stripplot(y='metric', hue='parcel',data=df, 
                  palette=cmaps[0], size=8, dodge=False, 
                  legend=False,ax = ax)
    
    # Labels and Ticks
    ax.set_xlabel('', fontsize=16)
    ax.set_xticklabels(['Parcels'], fontsize=16) #becasue of np.arrang does not include end of specified range
    ax.set_ylabel(metric_label, fontsize=16)
    ax.tick_params(axis='both', which='major', labelsize=14)
    ax.tick_params(axis='both', which='minor', labelsize=14)
    plt.locator_params(axis='y',nbins=5)
...
#%% Make Surface Plots of R-Squared

metrics = [rsq.mean(axis=0), rsq[group_list==1].mean(axis=0), rsq[group_list==2].mean(axis=0), rsq[group_list==3].mean(axis=0), rsq[group_list==4].mean(axis=0)]
metric_labels = ['All','HC1','PD','HC2','RBD']

group_labels, group_colors, cmaps , cropped_cmaps = prepare_plotting(inverse_cmp=False)
cmaps = [cmaps[0], cmaps[0], cmaps[1], cmaps[0], cmaps[3]]

for i, metric, metric_label in zip(range(5), metrics, metric_labels):
    old_power.save(
        metric,
        mask_file="MNI152_T1_8mm_brain.nii.gz",
        parcellation_file="Glasser52_binary_space-MNI152NLin6_res-8x8x8.nii.gz",
        asymmetric_data={'vmin': np.min(metrics), 'vmax': np.max(metrics)},
        plot_kwargs={
        ...
        filename=f"{plot_dir}/{metric_label}_rsqaured_.png",
    )
