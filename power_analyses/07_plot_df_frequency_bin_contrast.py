...
Plot frequency bin power group contrasts with GLMs accounting for confounds.
Permutation testing and max-tstatistic pooling accross frequency bins and parcels used to
calculate significance threshold. Threshold is only sued to aid visualisation but is not interpreted
in the manuscript, since it is very conservative.
Figure is presented in Figure 2 of the manuscript.
"""
...
# Set Dirs
datdir = '.../RBD_x_PD/Glasser52/rest/data/static/df_spect/contrasts'
outdir = '.../RBD_x_PD/Glasser52/rest/results/static/metrics/df_spect/glms'
...
# --- Load Freuqency bins ---
f = np.load(f"{metric_dir}/defooofed_f.npy")
f = f[f<=35]
...
for i, name in enumerate(["age","PD-HC",'RBD-HC','PD-RBD']):
   ...
   # ax[1].legend(bbox_to_anchor=(1, 1),fontsize = 12, frameon=False)
    ...
    plt.savefig(f'{outdir}/{name}_df_spect_glm_overview.svg',transparent = False, format='svg')
